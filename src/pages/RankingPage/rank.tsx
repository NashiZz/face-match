import { TableContainer, Paper, Table, TableBody, TableCell, TableHead, TableRow, Card, Button, Box } from "@mui/material";
...
function RankingPage() {
    ...
    return (
        <div style={{ display: "flex", justifyContent: "center", alignItems: "center", marginTop: 120 }}>
            <Card sx={{ width: 1550, height: 800, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center" }}>
                ...
                <TableContainer component={Paper} style={{ maxWidth: 1400, marginBottom: 20 }}>
                    <Table>
                        <TableHead>
                            <TableRow style={{  }}>
                                <TableCell sx={{ fontFamily: 'Kanit, sans-serif', fontSize: 16, fontWeight: "bold" }}>อันดับ</TableCell>
                                <TableCell sx={{ fontFamily: 'Kanit, sans-serif', fontSize: 16, fontWeight: "bold" }}>รูปภาพ</TableCell>
                                <TableCell sx={{ fontFamily: 'Kanit, sans-serif', fontSize: 16, fontWeight: "bold" }}>ชื่อรูป</TableCell>
                                <TableCell sx={{ fontFamily: 'Kanit, sans-serif', fontSize: 16, fontWeight: "bold" }}>คะแนนโหวต</TableCell>
                            </TableRow>
                        </TableHead>
                        <TableBody>
                            {rankingData.map((data, index) => (
                                <TableRow key={index}>
                                    <TableCell sx={{ fontFamily: 'Kanit, sans-serif', fontSize: 16, fontWeight: "bold" }} >{index + 1}</TableCell>
                                    <TableCell >
                                        <Box sx={{ position: 'relative', width: '80%', height: 0, paddingBottom: '80%', borderRadius: '10px', overflow: 'hidden', boxShadow: '0px 4px 10px rgba(0, 0, 0, 0.1)' }}>
                                            <img style={{ position: 'absolute', width: '100%', height: '100%', objectFit: 'cover' }} src={data.img} alt="" />
                                        </Box>
                                    </TableCell>
                                    <TableCell sx={{ fontFamily: 'Kanit, sans-serif', fontSize: 16 }}>{data.username}</TableCell>
                                    <TableCell sx={{ fontFamily: 'Kanit, sans-serif', fontSize: 16 }}>{data.score}</TableCell>
                                </TableRow>
                            ))}
                        </TableBody>
                    </Table>
